...
func _ready() -> void:
	...
	random.randomize()
	set_music_mode()
	GameManager.coin_overflow.connect(_on_game_manager_coin_overflow)
	...
	if GameManager.gem_probability / 16384 >= 1:
		...
		$Egg2.visible = true
		$Egg1.visible = true
	elif GameManager.gem_probability / 4096 >= 1:
		...
		$Egg2.visible = true
		$Egg1.visible = true
	elif GameManager.gem_probability / 1024 >= 1:
		...
		$Egg2.visible = true
		$Egg1.visible = true
	elif GameManager.gem_probability / 256 >= 1:
		change_texture_of_ground("res://textures/breakstone.jpg")
		$Egg4.visible = true
		$Egg3.visible = true
		$Egg2.visible = true
		$Egg1.visible = true
	elif GameManager.gem_probability / 64 >= 1:
		change_texture_of_ground("res://textures/rocks.jpg")
		$Egg3.visible = true
		$Egg2.visible = true
		$Egg1.visible = true
	elif GameManager.gem_probability / 16 >= 1:
		change_texture_of_ground("res://textures/snow.jpg")
		$Egg2.visible = true
		$Egg1.visible = true
	elif GameManager.gem_probability / 4 >= 1:
...
func _on_area_3d_input_event(camera: Node, event: InputEvent, event_position: Vector3, normal: Vector3, shape_idx: int) -> void:
	if event is InputEventScreenTouch or event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			GameManager.music_mode = (GameManager.music_mode - 1)
			if GameManager.music_mode < 0:
				GameManager.music_mode = 2
			set_music_mode()
...
func _on_slime_died() -> void:
	add_coins.start(GameManager.get_coin_count())
...
func _on_send_top_timer_timeout() -> void:
	if last_gems != GameManager.gems || last_coins != GameManager.coins:
